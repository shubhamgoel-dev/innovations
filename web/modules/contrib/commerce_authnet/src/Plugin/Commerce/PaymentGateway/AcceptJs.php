  ...
  public function createPayment(PaymentInterface $payment, $capture = TRUE) {
    ...
    if (isset($profiles['shipping']) && !$profiles['shipping']->get('address')->isEmpty()) {
      ...
      $ship_data = [
        ...
        'firstName' => substr($shipping_address->getGivenName(), 0, 50),
        'lastName' => substr($shipping_address->getFamilyName(), 0, 50),
        'address' => substr($shipping_address->getAddressLine1() . ' ' . $shipping_address->getAddressLine2(), 0, 60),
        'country' => substr($shipping_address->getCountryCode(), 0, 60),
        'company' => substr($shipping_address->getOrganization(), 0, 50),
        'city' => substr($shipping_address->getLocality(), 0, 40),
        'state' => substr($shipping_address->getAdministrativeArea(), 0, 40),
        'zip' => substr($shipping_address->getPostalCode(), 0, 20),
      ];
